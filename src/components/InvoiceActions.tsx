...
export default function InvoiceActions({ invoiceId, isPaid }: { invoiceId: string, isPaid: boolean }) {
  ...
  return (
    <div className="no-print" style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
      ...
      >
        {loading && <span className="btn-spinner" style={{ borderTopColor: isPaid ? '#991b1b' : '#166534' }} />}
      ...
      <button onClick={handleDelete} disabled={loading} className="btn btn-outline" style={{ color: 'red', borderColor: 'red', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
        🗑️ ลบ
      </button>
    </div>
  )
